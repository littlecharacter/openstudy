package com.lc.structure.linear.array.as;
...
public class SuffixArray_DC3 {
    ...
    // 3，max 是 nums 里面的最大值
    public SuffixArray_DC3(int[] nums, int max) {
        sa = sa(nums, max);
        rank = rank();
        height = height(nums);
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            SuffixArray_DC3 dc3 = new SuffixArray_DC3(arr, maxValue);
            long end = System.currentTimeMillis();
            System.out.println("数据量 " + len + ", 运行时间 " + (end - start) + " ms");

            // 倍增法 和 DC3算法 结果对比
            int[] sa = new SuffixArray_Doubling().sa(arr);
